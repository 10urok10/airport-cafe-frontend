...
`ModifiersModal`'s "Yeni Ekstra Ekle" form is a `MODIFIER_PRESETS`-driven `<select>`, not free text — picking e.g. "Yulaf Sutu" pre-fills its known `priceDelta` and, on submit, chains a second API call that auto-links the matching `Ingredient` (matched by exact name against `allIngredients`) at the preset's default `usageAmount`, so the admin never has to open the recipe sub-form separately for a known extra. "Ozel (elle giris)" falls back to the old plain-text-name, no-auto-link behavior for anything not in the preset list. Because this is now a two-step submit (create, then link) spanning two mutations, the button is guarded by its own local `isSubmitting` state rather than only the create mutation's `isPending` — the latter goes false as soon as step one resolves, which left a re-entrancy window between the two `await`s that a real double-click could (and during testing, did) hit.
