...
Cart lines need a third key dimension for this to work correctly: `cartLineKey(productId, variantId, linkedToKey)`, not just `productId+variantId`. Without `linkedToKey`, adding "Ekstra Shot" once from the Ekstralar tab (unlinked) and once via Americano's picker (linked) would collide into a single cart line and silently merge their quantities — since both have the same `productId` and no `variantId`. `linkedToKey` is simply the cart key of the base line the extra was added *for* (e.g. `"12-5-none"` for an Americano-Orta line), so the same extra product linked to two different base lines (or added unlinked) always gets its own line. Linked lines carry `linkedToName` (e.g. `"Airport Americano (Orta)"`) for the cart's own `↳ ... icin` display, plus `linkedToProductId`/`linkedToVariantId` — these two *are* sent to the backend (`handleSubmitOrder`'s payload includes them per item alongside `productId`/`variantId`/`quantity`), because the same "for X" connection needs to survive past order creation so `KitchenPage.jsx` can show it too (see below) — `linkedToKey`/`linkedToName` themselves stay UI-only, they're cart-session bookkeeping the backend has no use for.

### Showing a linked extra in the kitchen, not just the cart

An `OrderItem` returned by the backend (`GET /orders`, and therefore `KitchenPage.jsx`'s queries) can carry `linkedToProduct`/`linkedToVariant` (see backend `CLAUDE.md` — this is `OrderItem.linkedToProductId`/`linkedToVariantId`, a *different* mechanism from the `ProductExtra` catalog links `OrderPage.jsx` reads to build its picker). `KitchenPage.jsx`'s `OrderCard` mirrors `OrderPage.jsx`'s cart styling for these: an item with `linkedToProduct` set renders indented with a `↳` prefix and a "· {linkedToProduct.name} ({linkedToVariant.name}) icin" caption, so a barista glancing at the kitchen queue sees the same "Ekstra Shot goes with this Americano" grouping the order-taking screen showed when the item was added — without it, a kitchen card would just show two unrelated-looking lines and the connection would be lost the moment the order left `OrderPage.jsx`. Orders created before this field existed simply have `linkedToProduct: null` on every item and render as a flat list, exactly as before — no migration of historical data, nothing to reconcile.
