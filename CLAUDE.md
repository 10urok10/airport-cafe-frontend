...
`ModifiersModal`'s "Yeni Ekstra Ekle" form is a `<select>`, not free text — but the option list (`knownModifiers`) isn't hardcoded, it's derived every render from `allProducts` (all products, passed down from `ProductsPage`'s own `products` state, each already carrying its `.modifiers`): every distinct modifier `name` across every product becomes a selectable preset, minus whatever names the *current* product already has. Picking one pre-fills its `priceDelta` and, on submit, chains a second API call per linked ingredient that copies its `{ingredientId, usageAmount}` pairs onto the new modifier — no re-lookup by name needed, since `Ingredient` rows are global, not product-scoped, so an `ingredientId` from one product's modifier is valid to reuse directly on another's. "Ozel (elle giris)" falls back to plain-text-name, no-auto-link — and from that point on, whatever name was typed becomes a preset for every other product too, since it's now just another row `allProducts` will pick up. Because this is a two-step submit (create, then link) spanning two mutations, the button is guarded by its own local `isSubmitting` state rather than only the create mutation's `isPending` — the latter goes false as soon as step one resolves, which left a re-entrancy window between the two `await`s that a real double-click could (and during testing, did) hit.
